...
try {
  pgCreds = JSON.parse(
    fs.readFileSync("./firestore/supabase-service.json", "utf8")
  );
  if (
  ...
  return new Promise((resolve, reject) => {
    ...
    jsonStream.on("data", ({ key, value }) => {
      for (const attr in value) {
        if (attr === "firestore_id" && primary_key_strategy === "firestore_id")
          continue;
        if (!fields[attr]) {
...
async function loadData() {
  return new Promise((resolve, reject) => {
    try {
      ...
      resolve(runSQL(makeInsertStatement(fields, insertRows)));
      insertRows = [];
    } catch (err) {
      reject(err);
    }
